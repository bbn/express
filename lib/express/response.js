...
/**
 * Send a response of the given `data` as JSON, 
 * wrapping in a JS callback function if a callback parameter is present in the request `req`.
 *
 * Examples:
 *
 *     res.sendjsonp([1,2,3,4,5],req); 
 *       --> "[1,2,3,4,5]"
 *     res.sendjsonp({'table':87.99,'chair':12.99},req);
 *       --> 'mydata({"table":87.99,"chair":12.99})'
 *         (if callback=mydata is in the query string)
 *
 * @param {Object} data
...
http.ServerResponse.prototype.sendjsonp = function(data, req){
  var json = JSON.stringify(data);
  if (req.param('callback') !== undefined) {
    json = req.param('callback')+'('+json+')';
  }
  this.send(json);
}
